...
checkAccess(['admin']); // Admin y Cosecha tienen acceso a esta página
include 'db.php';
  ...
  <nav class="navbar navbar-expand-lg navbar-light bg-light">
    ...
    <section class="container">
        <nav class="nav flex-column nav-pills nav-fill">
            <li><a class="nav-link  active" href="TipoEmpleado.php">Cargos de personal</a></li>  
        ...
        <article style="height: 650px;">
                  <div class="jumbotron text-center ">
                    <h1 class="display-3 mb-4">Listado de Cargos de personal</h1>
                </div>
            ...
            try {
                ...
                // Procesar solicitud para eliminar tipo de empleado
                if (isset($_GET['delete_id'])) {
                    $delete_id = $_GET['delete_id'];
                    $sql = "DELETE FROM Tipos_Empleados WHERE id_tipo_empleado=:id_tipo_empleado";
                    $stmt = $conn->prepare($sql);
                    $stmt->execute([':id_tipo_empleado' => $delete_id]);
                }

                // Consulta para obtener los datos de la tabla Tipos_Empleados
                ...
                <tbody>
                    ...
                    if (!empty($result)) {
                        ...
                        foreach ($result as $row) {
                            echo "<tr>
                                    <td>" . htmlspecialchars($row["id_tipo_empleado"]) . "</td>
                                    <td>" . htmlspecialchars($row["descripcion"]) . "</td>
                                    <td>
                                        <a href='editar_tipo_empleado.php?id=" . htmlspecialchars($row["id_tipo_empleado"]) . "' class='btn btn-warning btn-sm'>Editar</a>
                                        <a href='?delete_id=" . htmlspecialchars($row["id_tipo_empleado"]) . "' class='btn btn-danger btn-sm'>Eliminar</a>
                                    </td>
                                  </tr>";
                        }
                    } else {
                        echo "<tr><td colspan='3'>No hay cargos registrados</td></tr>";
                    }
                    ?>
                </tbody>
            </table>
